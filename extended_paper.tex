...
%%     analysis (Sec.~\ref{sec:stability}); NEW EXPERIMENTS -- a multi-seed
%%     cross-architecture replication on Qwen2.5-Coder-1.5B, with its own
%%     research question RQ5, protocol (Sec.~\ref{sec:replication}) and
%%     results (Sec.~\ref{sec:cross-arch}, Tabs.~\ref{tab:cross-arch}
...
%%
%%  ON THE REPLICATION RESULT -- read before submitting. The new experiments
%%  do not simply confirm the conference paper. Hybrid-CASR's forward-F1
%%  lead does NOT reproduce on the second backbone (-0.006 vs the published
%%  +0.016); its backward-retention advantage and the ranking of the weaker
%%  methods DO. The text reports this as measured and reframes the method's
%%  contribution as a retention/accuracy trade. This is deliberate and, we
%%  believe, the stronger paper -- but it is a change of claim relative to
%%  the conference version and all three authors should agree to it.
%%
%%  ALSO NOTE the "Metric definition" paragraph in Threats to Validity: the
%%  reference implementation computes sklearn's default binary F1 (positive
%%  class only), not a macro average. Every number in both the conference
%%  and the extended version is on that same footing, so no comparison or
%%  ranking changes, but the paragraph states it explicitly. Keeping it is
%%  the authors' call.
...
\begin{itemize}
    ...
    \item \textbf{RQ5 (new):} Which of these findings are properties of continual-learning strategy, and which are properties of the particular backbone they were measured on?
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Cross-architecture replication (new).} A multi-seed re-run of the core comparison on a second backbone (\texttt{Qwen2.5-Coder-1.5B}), which separates the findings that transfer from the one that does not: the ranking of the weaker strategies and Hybrid-CASR's retention advantage both hold, while its forward-accuracy lead does not reproduce.
...
\noindent
The sharpest threat to the conference study was that every conclusion rested on a single backbone. This extended version therefore re-runs the core comparison on a second, deliberately dissimilar decoder and asks whether the ranking survives. This subsection describes that replication; its results are reported in Section~\ref{sec:cross-arch}.

\textbf{Second backbone.} We use \texttt{Qwen2.5-Coder-1.5B}: roughly half the parameter count of phi-2, a different pretraining corpus and tokeniser, and---unlike the general-purpose phi-2---a backbone specialised on code from the outset. The pairing is intentionally adversarial to our own result. If Hybrid-CASR's advantage were a property of selective replay rather than of one model, the smaller code-specialised model is where it should show most clearly, since it has the least general capacity to fall back on.
...
\noindent
Everything above rests on one backbone. Following the protocol of Section~\ref{sec:replication}, we now re-run the core comparison on \texttt{Qwen2.5-Coder-1.5B} and ask which of the conclusions are properties of selective replay and which were properties of phi-2. This subsection reports the principal new experiments of the extended version (RQ5).

\textbf{Reading the tables.} One caveat governs everything that follows. Re-running the window-only baseline on phi-2 over the rebuilt corpus and the current dependency stack gives $0.7236$, not the $0.651$ published in the conference version. The gap is not small, and we did not attribute it: the rebuilt corpus, the move from \texttt{transformers}~4.35 to~5.15 and from \texttt{peft}~0.6 to~0.20 all changed at once. Its consequence for this section is procedural rather than substantive. The replication is internally consistent---one corpus, one code base, one dependency set, one hardware configuration across all of its runs---so comparisons \emph{within} Tables~\ref{tab:cross-arch} and~\ref{tab:cross-arch-ibr} are sound, but individual cells must not be read against Tables~\ref{tab:method-forward} or~\ref{tab:retention-detailed}. Where we compare against a conference result below, we compare \emph{differences between methods}, never absolute levels.

\begin{table}[t]
  \centering
  \caption{Forward F1 under the cross-architecture replication. $\Delta$ and $p$ are paired Wilcoxon signed-rank tests against the window-only baseline \emph{on the same backbone}, over the 41 common windows, after averaging across seeds. Newly added in this extended version.}
...
\textbf{The forward advantage does not survive the backbone change.} On phi-2 the conference version measured Hybrid-CASR ahead of window-only by $+0.016$. On Qwen2.5-Coder-1.5B the same comparison gives $-0.0056$ (Table~\ref{tab:cross-arch}). Two facts must be reported together here, because either alone misleads. The difference is significant under a window-paired Wilcoxon test ($p=0.032$); but it is \emph{smaller than Hybrid-CASR's own seed-to-seed standard deviation} ($0.0066$), and the two methods' seed ranges overlap ($0.6780$--$0.6868$ against $0.6732$--$0.6850$). The defensible reading is therefore that the forward advantage \emph{fails to reproduce} on a second architecture---not that it significantly reverses. Reporting the $p$-value without the seed overlap would be selective presentation of our own result.

The seed sweep carries a methodological point of its own. Computed from a single seed, this same comparison gives $-0.0136$ at $p=0.0022$; adding two more seeds halves the effect to $-0.0056$ at $p=0.032$. An effect that shrinks by half as replication increases is an effect estimated at the edge of what a single run can resolve---and that caution applies with equal force to the conference version's original $+0.016$, which was also a single-run estimate.

\begin{table}[t]
  \centering
  \caption{Immediate Backward Retention under the cross-architecture replication. Decay $=(\mathrm{IBR@1}-\mathrm{IBR@6})/\mathrm{IBR@1}$. Absolute levels are not comparable with Table~\ref{tab:retention-detailed} (see text). Newly added in this extended version.}
...
\textbf{Backward retention behaves in the opposite direction, and it does transfer.} On the new backbone Hybrid-CASR leads window-only at \emph{all four} lags, and the margin decays monotonically with temporal distance: $+0.028$ at lag 1, $+0.016$ at lag 3, $+0.006$ at lag 5, $+0.002$ at lag 6 (Table~\ref{tab:cross-arch-ibr}). That shape is exactly what the mechanism predicts, since the buffer draws from $W_{t-1}$ and $W_{t-2}$ and its protective effect should therefore fade as the probe moves out of buffer range. Taken with the forward result, the honest summary of Hybrid-CASR on a second architecture is a \emph{trade}: it buys a measurable improvement in retention at a forward cost that is statistically indistinguishable from zero at the resolution of three seeds. That is a narrower claim than the conference version's, and it sits squarely inside the stability--plasticity frame the paper already uses---which is arguably a better home for it than an unqualified accuracy win.

\textbf{The bottom of the ranking is architecture-independent.} Replay-3P ($-0.0371$) and OLoRA ($-0.0884$) remain the two weakest methods on Qwen, in the same order as on phi-2 ($0.622$ and $0.599$ in Table~\ref{tab:method-forward}). Both deficits are six to twenty times the seed spread we measured, so a single seed suffices to establish them. Whatever is wrong with these two strategies is not a property of the backbone.

\textbf{A mechanistic reading of Replay-3P.} Its retention curve is the most informative single row in Table~\ref{tab:cross-arch-ibr}. Despite finishing second-to-last on forward F1, Replay-3P posts an IBR@1 of $0.8936$---above the window-only baseline ($0.8799$) and behind only Hybrid-CASR ($0.9081$)---and then falls to $0.6446$ at lag 3, a drop of $0.25$ that no other method exhibits. The break falls precisely at the edge of its replay horizon: it rehearses $W_{t-1}$ and $W_{t-2}$, and its retention collapses at the first probe that lies outside them. The natural reading is that Replay-3P's apparent retention is supplied by the replayed data itself rather than by knowledge accumulated in its parameters. Methods that carry adapter state forward across windows---window-only and Hybrid-CASR---instead decay smoothly, with no discontinuity at any particular lag. Retention measured at short lags therefore cannot distinguish a model that has \emph{remembered} from one that has merely been \emph{re-shown}, and probing at several lags, as we do, is what separates them.

\textbf{An observation we decline to turn into a conclusion.} The larger backbone retains better: phi-2 decays $8.5\%$ from lag 1 to lag 6 against Qwen's $14.3\%$, and leads on forward F1 as well ($0.7236$ against $0.6829$). It is tempting to read this as a scale effect. We do not, for two reasons: the phi-2 arm has a single seed, and the two models differ simultaneously in parameter count, pretraining corpus and architecture family, so nothing in this design separates those three. We report the observation and leave the attribution to a study that varies size within one model family.
...
The cross-architecture replication (Section~\ref{sec:cross-arch}) pushes that qualification one step further, and in a direction we did not choose. On a second backbone the forward-accuracy lead is gone---$-0.0056$ where phi-2 gave $+0.016$, and small enough that the two methods' seed ranges overlap. What survives the change of architecture is the retention advantage, present at every lag, and the ordering of the weaker strategies. The three findings compose into a single coherent statement rather than a contradiction: Hybrid-CASR's forward gain was always inside the top cluster's noise band, as Section~\ref{sec:pairwise} had already indicated on phi-2 alone, and a quantity that is statistically indistinguishable from its neighbours on one architecture is precisely the quantity that should not be expected to reproduce on another. The retention effect, which is mechanistically tied to what the buffer holds, does reproduce. We therefore state the method's contribution as a trade---retention bought at negligible forward cost---rather than as an accuracy improvement, and we regard the reframing as the single most useful outcome of this extension.
...
The ranking among replay methods is itself informative. Replay-1P tops immediate retention yet, as Section~\ref{sec:stability} shows, is among the most volatile, and Replay-3P underperforms despite holding more history---contradicting the intuition that bigger buffers monotonically help~\citep{Han2023CLCompare} and suggesting that, under fast drift, \emph{controlled} forgetting beats comprehensive retention.

The lagged retention profiles of Section~\ref{sec:cross-arch} add a distinction that the buffer-size framing alone cannot draw. Replay-3P's retention holds up inside its replay horizon and collapses immediately outside it, whereas methods that carry adapter state forward decay smoothly across all four lags. Two very different mechanisms can therefore produce a respectable IBR@1: knowledge genuinely consolidated in the parameters, and data simply shown again a window ago. They are indistinguishable at short lags and separate cleanly at long ones. This has a direct methodological consequence for continual-learning evaluation on code---single-lag retention metrics, still common in the literature, cannot support claims about consolidation---and it suggests that Replay-3P's weakness may be less about buffer size than about how little of what it rehearses is retained once rehearsal stops. OLoRA's weakness ($0.599$) similarly cautions against importing strict orthogonality constraints---successful in vision~\citep{LopezPaz2017GEM} and some LLM settings~\citep{Song2023ConPET}---into a domain where vulnerability types overlap and recur across time, so that orthogonalising away past directions also erases still-relevant signal.
...
The replication sharpens this advice rather than overturning it. On a second backbone the accuracy argument for Hybrid-CASR disappears entirely, leaving retention, stability and cost as the whole of the case. That is still a sound case, but it is a conditional one: a team should adopt selective replay if old vulnerability patterns recur in its code base and being able to catch them a year later has value, and should be content with per-window fine-tuning if it does not. The corollary for anyone evaluating such a choice locally is that a difference of a few thousandths of F1 measured from one training run should not be acted on---ours halved when we replicated it across seeds.
...
\textbf{Architecture coverage.} The conference version rested entirely on phi-2. Section~\ref{sec:cross-arch} removes part of that exposure by replicating the core comparison on \texttt{Qwen2.5-Coder-1.5B}, and the outcome vindicates the concern: the ranking of the weaker strategies transfers, but Hybrid-CASR's forward-accuracy lead does not. Two decoders are still two decoders, however. Nothing here speaks to encoders such as CodeBERT~\citep{Feng2020CodeBERT}, to graph models~\citep{Zhou2019Devign,Cao2021BGNN4VD}, or to backbones past the single-GPU range, and the granularity sweep and the top-cluster equivalence of Section~\ref{sec:pairwise} were not re-run on the second backbone.
...
Fifth, and most substantively, we add an entirely new experimental arm: a multi-seed cross-architecture replication on \texttt{Qwen2.5-Coder-1.5B} (protocol in Section~\ref{sec:replication}, results in Section~\ref{sec:cross-arch}, Tables~\ref{tab:cross-arch} and~\ref{tab:cross-arch-ibr}), together with the new research question RQ5 that motivates it. This is the one addition that could have contradicted the conference version, and in part it does: Hybrid-CASR's forward-accuracy lead does not reproduce on the second backbone, whereas its backward-retention advantage and the ordering of the weaker strategies do. We report that outcome as measured, and Section~\ref{sec:disc-interpretation} argues that it does not so much overturn the conference conclusion as locate it correctly---a lead that Section~\ref{sec:pairwise} had already shown to lie inside the top cluster's noise band is not one that should have been expected to survive a change of architecture. The replication also produced a finding with no counterpart in the conference version: lagged retention profiles distinguish knowledge consolidated in parameters from data merely rehearsed recently (Section~\ref{sec:disc-theory}).

Sixth, the threats-to-validity discussion is broadened accordingly, with new treatments of pretraining contamination, the rebuilt corpus and its measured deviation from the published one, the breadth of the replication, input truncation, and the exact definition of the reported F1. Finally, the title, abstract and conclusions have been rewritten to reflect the shift in emphasis---from proposing a method toward establishing, with fuller statistics and a second architecture, what selective replay can and cannot be relied on to deliver under temporal drift. Tables and figures carried over from the conference version are marked as reproduced at each caption; all newly added tables are marked as such.
...
The cross-architecture replication then tells us which parts of this survive contact with a different model, and the answer is instructive precisely because it is mixed. On Qwen2.5-Coder-1.5B the forward-accuracy lead does not reproduce---$-0.006$, inside the seed-to-seed spread of both methods---while the backward-retention advantage holds at every lag and the ordering of the weaker strategies is unchanged. Read together with the significance analysis, the two results agree rather than conflict: a difference that sits inside a noise band on one architecture is not one that should be expected to transfer to another, and it was the retention effect, tied to what the replay buffer actually holds, that had a mechanism behind it. We therefore state the contribution of Hybrid-CASR as a \emph{trade}---measurably better retention at a forward cost indistinguishable from zero, together with the steadiest and among the cheapest operating points of the top cluster---rather than as an accuracy improvement. That is a smaller claim than the conference version's, and a considerably more durable one.

Two further lessons generalise beyond this method. First, effect sizes of a few thousandths of F1 are not estimable from single runs: ours halved when replication went from one seed to three, which is reason enough to treat single-run comparisons at this scale, our own earlier one included, as provisional. Second, retention probed at a single short lag cannot distinguish consolidation from rehearsal---a fixed-horizon replay method held its ground inside its buffer window and collapsed by $0.25$ F1 immediately beyond it, while methods carrying adapter state forward decayed smoothly throughout. The granularity sweep adds a complementary note of robustness: one-month to twelve-month windows land within a $0.651$--$0.669$ band, so time scale changes \emph{which} vulnerabilities are caught more than \emph{how many}.
